...
def _numerical_sort_key(path):
  # extract the number from 'color_123.png'
...
class YcbineoatReader:
  def __init__(self,video_dir, downscale=1, shorter_side=None, zfar=np.inf):
    ...
    self.zfar = zfar
    # numerically sort color files
    self.color_files = sorted(
        glob.glob(f"{self.video_dir}/rgb/*.png"),
        key=_numerical_sort_key
    )
    # same for depth
    self.depth_files = sorted(
        glob.glob(f"{self.video_dir}/depth/*.png"),
        key=_numerical_sort_key
    )
  ...
  def get_mask(self,i):
    color_fn = os.path.basename(self.color_files[i])             # e.g. 'color_1.png'
    mask_fn  = color_fn.replace("color_", "mask_00")               # 'mask_1.png'
    mask_fp  = os.path.join(self.video_dir, "masks", mask_fn)    # …/masks/mask_1.png

    mask = cv2.imread(mask_fp, -1)
    if mask is None:
      raise FileNotFoundError(f"Mask not found: {mask_fp}")
    # If it’s RGB, pick any non-zero channel
    if mask.ndim == 3:
  ...
  def get_depth(self,i):
    depth = cv2.imread(self.depth_files[i], -1) / 1e3
