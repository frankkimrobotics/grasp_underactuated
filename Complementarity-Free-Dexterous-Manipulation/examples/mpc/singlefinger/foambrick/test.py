...
from examples.mpc.singlefinger.foambrick.params import ExplicitMPCParams
...
save_flag=True
if save_flag:
    save_dir = './examples/mpc/singlefinger/foambrick_w_planner/save/'
    prefix_data_name = 'ours_'
    save_matrix = []
    save_data = dict()
    save_row = 0
    data_name= 'foambrick_w_planner_singlefingerdata_varying:MPC,pos_weight,trials'
...
mpc_horizons = [4,5,6,7,8,10]
pos_weights = [0.005,0.01,0.02,0.035,0.05,0.01]
used_pos_weights = []
used_mpc_horizons = []
...
while trial_count < trial_num:
    for pos_weight in pos_weights:
        for mpc_horizon in mpc_horizons:
            ...
            #        init parameters
            # -------------------------------
            param = ExplicitMPCParams(rand_seed=trial_count, target_type='ground-flip',mpc_horizon=mpc_horizon,pos_weight=pos_weight)
            ...
            while rollout_step < max_rollout_length:
                if not env.dyn_paused_:
                    ...
                    if stuck_count > max_stuck_steps:
                        ...
                        new_robot_pos = last_obj_pos + offset_dir
