...
class ExplicitMPCParams:
    def __init__(self, init_pose = None, target_pose = None, rand_seed=1, target_type=True,mpc_horizon =10,pos_weight=0.1):
        # ---------------------------------------------------------------------------------------------
        #      simulation parameters 
        # ---------------------------------------------------------------------------------------------
        # UPDATED model path to single-fingertip environment (if you have a different XML file).
        self.model_path_ = 'envs/xmls/env_singlefinger_mustard.xml'
        ...
        if init_pose is None:
            init_height = 0.03
            init_xy_rand = 0.35 * np.random.rand(2) - 0.025
            ...
            self.init_obj_qpos_ = init_pose

        # Single fingertip initial qpos (x,y,z)
        # You can choose whatever 3D starting point you want:
        self.init_robot_qpos_ = np.random.rand(3)*0.1
        # self.init_robot_qpos_ = np.array([0.2, 0.0, 0.0])  # For example

        # random target pose for object
        if target_type == 'ground-rotation':
            if target_pose is None:
                target_xy_rand = 0.25 * np.random.rand(2) - 0.1
                self.target_p_ = np.hstack([target_xy_rand, 0.035])  # Set height slightly above ground
                yaw_angle = 2 * np.pi * np.random.rand(1) - np.pi
                # Set pitch and roll to 0 for stable ground contact
                self.target_q_ = rotations.rpy_to_quaternion(np.hstack([yaw_angle, np.pi-np.pi/30, np.pi/2]))
            else:
                self.target_p_, self.target_q_ = target_pose

        elif target_type == 'ground-flip':
            target_xy_rand = 0.25 * np.random.rand(2) - 0.1
            self.target_p_ = np.hstack([target_xy_rand, 0.03])  # Set height slightly above ground
            yaw_angle = 2 * np.pi * np.random.rand(1) - np.pi
            pitch_angle = 2 * np.pi * np.random.rand(1) - np.pi
            roll_angle = 2 * np.pi * np.random.rand(1) - np.pi
            # Set pitch and roll to 0 for stable ground contact
        ...
        fts_q_lb = np.array([-100, -100, 0.015])
    ...
    def init_cost_fns(self):
        ...
        # pp = 0.4
        # final_cost = (pp)*(500.0 * position_cost) + (1-pp)*(50.0 * quaternion_cost)
        # print(pp)
        # print(self.pos_weight)

        final_cost = (self.pos_weight)*(1.0 * position_cost) + (1-self.pos_weight)*(1* quaternion_cost)
        print(f"THE POSITION WEIGHT USED IN COST: {self.pos_weight}")
