...
import math
import mathutils
import argparse, pathlib, blenderproc as bproc
...
# Camera Setup
radius = 6
min_lat = 0; max_lat = 70; step_lat = 70 # Latitude
min_lon = 0; max_lon = 360; step_lon = 120 # Longitude
...
def load_obj_with_texture(path, tex, pose, scale):
    obj = bproc.loader.load_obj(path)[0]
    obj.set_location(pose[:3])
    obj.set_rotation_euler(pose[3:])
    obj.set_scale(scale)
    mat = bproc.material.create(name="tex")
    mat.make_single_image_texture(tex)
...
def main(args):
    ...
    # quick sanity-check on the files you handed in from launcher
    if not pathlib.Path(args.obj_path).is_file():
        raise FileNotFoundError(args.obj_path)
    if not pathlib.Path(args.tex_path).is_file():
        raise FileNotFoundError(args.tex_path)

    create_box(*args.box_scale)

    pose = random_position_within_open_box(
        [0,0,args.box_scale[2]],  # box centre
        args.box_scale)
    load_obj_with_texture(args.obj_path, args.tex_path, pose, args.obj_scale)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--tex_path",   required=True)
    ap.add_argument("--box_scale",  nargs=3, type=float, default=[1.3,1.5,0.5])
    ap.add_argument("--obj_scale",  nargs=3, type=float, default=[3,3,3])
